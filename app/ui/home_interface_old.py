...
class ItemCardView(QWidget,Translator):
    ...
    def loadItemsByScrollBar(self,value):
        if value/(self.scrollArea.verticalScrollBar().maximum()+0.1) >= 0.98:
            self.load_more_itmes()
    def load_more_itmes(self):
        if not self.is_loading:
            self.is_loading = True
            self.worker = CommonWorker(self)
            self.worker.fun = self.load_more_items_thread
            self.worker.run()
    def load_more_items_thread(self):
    ...
    def editItem(self,index:int):
        ...
        editInterface.loadDataFromModelindex(libraryAssetData)
        editInterface.show()
